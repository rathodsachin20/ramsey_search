...
/*
  ...
  */
  ...
  puts("Best graph received from server");
  close(connfd);
  return 0;
  
}

int gread_from_file(char *fname){
  FILE* fp;
  fp = fopen(fname,"r");
  if(!fp){
    puts("oops");
...
int main(int argc, char *argv[]){
  int i=0;
  while(1){
    puts("reading graph from file...\n");
    //int bc = gread_from_file("newfile");
    for(i=0; i<100; i++){
      //if(i==bc){
        char fname[32];
        ...
        if(!gwrite_to_file(i,fname)){
          printf("write to file failed for fname: %d\n",i);
          break;
        }

        //send graph to server
        puts("sending graph to server...");
        if(!c_gsend(fname)){
          printf("sending failed for fname: %d",i);
          break;
        }
        puts("waiting to receive graph from server...");
        c_grecv();
        sleep(5);
        puts("*************************");
        break;
